...
def run_simulation(method='baseline'):
    ...
    for epoch in pbar:
        ...
        acc, loss = test_model(global_model, test_data)
    ...
    return acc_history, loss_history, time_history, energy_history


if __name__ == '__main__':

    acc_baseline, loss_base, time_base, energy_base = run_simulation(method='fedavg')
    acc_salf, loss_salf, time_salf, energy_salf = run_simulation(method='salf')
    acc_dmfl, loss_dmfl, time_dmfl, energy_dmfl = run_simulation(method='dmfl')
    ...
    axes[0].plot(epochs, acc_dmfl, 'b-s', label='DMFL (Ours)')
    ...
    # 子图2: Loss变化对比
    axes[1].plot(epochs, loss_base, 'r--o', label='fedavg')
    axes[1].plot(epochs, loss_salf, 'g-^', label='SALF')
    axes[1].plot(epochs, loss_dmfl, 'b-s', label='DMFL (Ours)')
    if args.warmup_rounds > 0:
    ...
    axes[1].set_title('Test Loss vs. Rounds')
    axes[1].legend()
    axes[1].grid(True)

    # 子图3: 累计时延对比
    axes[2].plot(epochs, time_base, 'r--o', label='Baseline')
    axes[2].plot(epochs, time_salf, 'g-^', label='SALF')
    axes[2].plot(epochs, time_dmfl, 'b-s', label='DMFL (Ours)')
    axes[2].set_xlabel('Global Communication Rounds')
    axes[2].set_ylabel('Cumulative Time Delay (s)')
    axes[2].set_title('Time Cost Comparison')
    axes[2].legend()
    axes[2].grid(True)

    # 子图4: 累计UE设备功耗对比
    axes[3].plot(epochs, energy_base, 'r--o', label='Baseline')
    axes[3].plot(epochs, energy_salf, 'g-^', label='SALF')
    axes[3].plot(epochs, energy_dmfl, 'b-s', label='DMFL (Ours)')
    axes[3].set_xlabel('Global Communication Rounds')
    axes[3].set_ylabel('Cumulative UE Energy (Joules)')
    axes[3].set_title('UE Device Power Consumption')
    axes[3].legend()
    axes[3].grid(True)
